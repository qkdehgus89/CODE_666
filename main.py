...
def init_db():
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS hunmin_initials (
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS hunmin_games (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        source_id TEXT NOT NULL,
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS hunmin_answers (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        game_id INTEGER NOT NULL,
        source_id TEXT NOT NULL,
        user_id TEXT NOT NULL,
    ...
    cur.execute("SELECT value FROM system_flags WHERE key = 'code666_caution_roster_seed_20260706_v1'")
    caution_seed_done = cur.fetchone()
    if not caution_seed_done:
        created_at = now_str()

        def caution_seed_key(name, birth, region):
            raw = f"{name}|{birth}|{region}"
            return re.sub(r"[^0-9A-Za-z가-힣]+", "", raw)
        ...
        seen_caution_keys = set()
        for name, birth, region, group_label in caution_rows:
            key = caution_seed_key(name, birth, region)
            if not key or key in seen_caution_keys:
                continue
            seen_caution_keys.add(key)
            display_name = f"{name}{birth} {region}".strip()
    ...
    cur.execute("SELECT value FROM system_flags WHERE key = 'hunmin_initial_seed_20260830_v1'")
    hunmin_seed_done = cur.fetchone()
    if not hunmin_seed_done:
        created_at = now_str()
        for initial in HUNMIN_INITIALS:
            cur.execute("""
            INSERT OR IGNORE INTO hunmin_initials (
                initial, is_active, created_by, created_at, updated_at
            ) VALUES (?, 1, 'system', ?, ?)
            """, (initial, created_at, created_at))
        cur.execute(
            "INSERT INTO system_flags (key, value) VALUES ('hunmin_initial_seed_20260830_v1', ?)",
            (created_at,)
        )
...
FORTUNE_COOKIE_MESSAGES = [
    ...
    '마음먹은 순간부터 길은 만들어집니다.',
    '오늘의 주인공은 바로 당신입니다.',
    '아침의 가벼운 결심이 당신의 하루에 좋은 방향을 열어줍니다.',
    '아침의 가벼운 결심이 예상보다 따뜻한 결과로 이어집니다.',
    '아침의 가벼운 결심이 마음속 부담을 조금 덜어줍니다.',
    '아침의 가벼운 결심이 새로운 가능성을 조용히 데려옵니다.',
    '아침의 가벼운 결심이 기분 좋은 자신감으로 남습니다.',
    '아침의 가벼운 결심이 다음 선택을 더 가볍게 만들어줍니다.',
    '아침의 가벼운 결심이 주변 사람과의 흐름을 부드럽게 합니다.',
    '아침의 가벼운 결심이 생각보다 오래 힘이 되어줍니다.',
    '아침의 가벼운 결심이 오늘의 운을 한층 밝게 만듭니다.',
    '아침의 가벼운 결심이 필요한 순간에 좋은 힌트가 됩니다.',
    '점심 무렵의 작은 선택이 당신의 하루에 좋은 방향을 열어줍니다.',
    '점심 무렵의 작은 선택이 예상보다 따뜻한 결과로 이어집니다.',
    '점심 무렵의 작은 선택이 마음속 부담을 조금 덜어줍니다.',
    '점심 무렵의 작은 선택이 새로운 가능성을 조용히 데려옵니다.',
    '점심 무렵의 작은 선택이 기분 좋은 자신감으로 남습니다.',
    '점심 무렵의 작은 선택이 다음 선택을 더 가볍게 만들어줍니다.',
    '점심 무렵의 작은 선택이 주변 사람과의 흐름을 부드럽게 합니다.',
    '점심 무렵의 작은 선택이 생각보다 오래 힘이 되어줍니다.',
    '점심 무렵의 작은 선택이 오늘의 운을 한층 밝게 만듭니다.',
    '점심 무렵의 작은 선택이 필요한 순간에 좋은 힌트가 됩니다.',
    '오후에 스친 생각이 당신의 하루에 좋은 방향을 열어줍니다.',
    '오후에 스친 생각이 예상보다 따뜻한 결과로 이어집니다.',
    '오후에 스친 생각이 마음속 부담을 조금 덜어줍니다.',
    '오후에 스친 생각이 새로운 가능성을 조용히 데려옵니다.',
    '오후에 스친 생각이 기분 좋은 자신감으로 남습니다.',
    '오후에 스친 생각이 다음 선택을 더 가볍게 만들어줍니다.',
    '오후에 스친 생각이 주변 사람과의 흐름을 부드럽게 합니다.',
    '오후에 스친 생각이 생각보다 오래 힘이 되어줍니다.',
    '오후에 스친 생각이 오늘의 운을 한층 밝게 만듭니다.',
    '오후에 스친 생각이 필요한 순간에 좋은 힌트가 됩니다.',
    '저녁의 차분한 마음이 당신의 하루에 좋은 방향을 열어줍니다.',
    '저녁의 차분한 마음이 예상보다 따뜻한 결과로 이어집니다.',
    '저녁의 차분한 마음이 마음속 부담을 조금 덜어줍니다.',
    '저녁의 차분한 마음이 새로운 가능성을 조용히 데려옵니다.',
    '저녁의 차분한 마음이 기분 좋은 자신감으로 남습니다.',
    '저녁의 차분한 마음이 다음 선택을 더 가볍게 만들어줍니다.',
    '저녁의 차분한 마음이 주변 사람과의 흐름을 부드럽게 합니다.',
    '저녁의 차분한 마음이 생각보다 오래 힘이 되어줍니다.',
    '저녁의 차분한 마음이 오늘의 운을 한층 밝게 만듭니다.',
    '저녁의 차분한 마음이 필요한 순간에 좋은 힌트가 됩니다.',
    '잠들기 전 떠오른 아이디어가 당신의 하루에 좋은 방향을 열어줍니다.',
    '잠들기 전 떠오른 아이디어가 예상보다 따뜻한 결과로 이어집니다.',
    '잠들기 전 떠오른 아이디어가 마음속 부담을 조금 덜어줍니다.',
    '잠들기 전 떠오른 아이디어가 새로운 가능성을 조용히 데려옵니다.',
    '잠들기 전 떠오른 아이디어가 기분 좋은 자신감으로 남습니다.',
    '잠들기 전 떠오른 아이디어가 다음 선택을 더 가볍게 만들어줍니다.',
    '잠들기 전 떠오른 아이디어가 주변 사람과의 흐름을 부드럽게 합니다.',
    '잠들기 전 떠오른 아이디어가 생각보다 오래 힘이 되어줍니다.',
    '잠들기 전 떠오른 아이디어가 오늘의 운을 한층 밝게 만듭니다.',
    '잠들기 전 떠오른 아이디어가 필요한 순간에 좋은 힌트가 됩니다.',
    '오래 미뤄둔 연락이 당신의 하루에 좋은 방향을 열어줍니다.',
    '오래 미뤄둔 연락이 예상보다 따뜻한 결과로 이어집니다.',
    '오래 미뤄둔 연락이 마음속 부담을 조금 덜어줍니다.',
    '오래 미뤄둔 연락이 새로운 가능성을 조용히 데려옵니다.',
    '오래 미뤄둔 연락이 기분 좋은 자신감으로 남습니다.',
    '오래 미뤄둔 연락이 다음 선택을 더 가볍게 만들어줍니다.',
    '오래 미뤄둔 연락이 주변 사람과의 흐름을 부드럽게 합니다.',
    '오래 미뤄둔 연락이 생각보다 오래 힘이 되어줍니다.',
    '오래 미뤄둔 연락이 오늘의 운을 한층 밝게 만듭니다.',
    '오래 미뤄둔 연락이 필요한 순간에 좋은 힌트가 됩니다.',
    '우연히 들은 한마디가 당신의 하루에 좋은 방향을 열어줍니다.',
    '우연히 들은 한마디가 예상보다 따뜻한 결과로 이어집니다.',
    '우연히 들은 한마디가 마음속 부담을 조금 덜어줍니다.',
    '우연히 들은 한마디가 새로운 가능성을 조용히 데려옵니다.',
    '우연히 들은 한마디가 기분 좋은 자신감으로 남습니다.',
    '우연히 들은 한마디가 다음 선택을 더 가볍게 만들어줍니다.',
    '우연히 들은 한마디가 주변 사람과의 흐름을 부드럽게 합니다.',
    '우연히 들은 한마디가 생각보다 오래 힘이 되어줍니다.',
    '우연히 들은 한마디가 오늘의 운을 한층 밝게 만듭니다.',
    '우연히 들은 한마디가 필요한 순간에 좋은 힌트가 됩니다.',
    '정리해둔 메모 하나가 당신의 하루에 좋은 방향을 열어줍니다.',
    '정리해둔 메모 하나가 예상보다 따뜻한 결과로 이어집니다.',
    '정리해둔 메모 하나가 마음속 부담을 조금 덜어줍니다.',
    '정리해둔 메모 하나가 새로운 가능성을 조용히 데려옵니다.',
    '정리해둔 메모 하나가 기분 좋은 자신감으로 남습니다.',
    '정리해둔 메모 하나가 다음 선택을 더 가볍게 만들어줍니다.',
    '정리해둔 메모 하나가 주변 사람과의 흐름을 부드럽게 합니다.',
    '정리해둔 메모 하나가 생각보다 오래 힘이 되어줍니다.',
    '정리해둔 메모 하나가 오늘의 운을 한층 밝게 만듭니다.',
    '정리해둔 메모 하나가 필요한 순간에 좋은 힌트가 됩니다.',
    '가볍게 시작한 일이 당신의 하루에 좋은 방향을 열어줍니다.',
    '가볍게 시작한 일이 예상보다 따뜻한 결과로 이어집니다.',
    '가볍게 시작한 일이 마음속 부담을 조금 덜어줍니다.',
    '가볍게 시작한 일이 새로운 가능성을 조용히 데려옵니다.',
    '가볍게 시작한 일이 기분 좋은 자신감으로 남습니다.',
    '가볍게 시작한 일이 다음 선택을 더 가볍게 만들어줍니다.',
    '가볍게 시작한 일이 주변 사람과의 흐름을 부드럽게 합니다.',
    '가볍게 시작한 일이 생각보다 오래 힘이 되어줍니다.',
    '가볍게 시작한 일이 오늘의 운을 한층 밝게 만듭니다.',
    '가볍게 시작한 일이 필요한 순간에 좋은 힌트가 됩니다.',
    '마음에 남은 조언이 당신의 하루에 좋은 방향을 열어줍니다.',
    '마음에 남은 조언이 예상보다 따뜻한 결과로 이어집니다.',
    '마음에 남은 조언이 마음속 부담을 조금 덜어줍니다.',
    '마음에 남은 조언이 새로운 가능성을 조용히 데려옵니다.',
    '마음에 남은 조언이 기분 좋은 자신감으로 남습니다.',
    '마음에 남은 조언이 다음 선택을 더 가볍게 만들어줍니다.',
    '마음에 남은 조언이 주변 사람과의 흐름을 부드럽게 합니다.',
    '마음에 남은 조언이 생각보다 오래 힘이 되어줍니다.',
    '마음에 남은 조언이 오늘의 운을 한층 밝게 만듭니다.',
    '마음에 남은 조언이 필요한 순간에 좋은 힌트가 됩니다.',
]
...
CODE_COOKIE_MESSAGES = [
    ...
    '공창에 남긴 첫 인사가 CODE 안에서 새로운 설렘으로 이어질 수 있습니다.',
    '공창에 남긴 첫 인사가 누군가에게 오래 기억될 수 있습니다.',
    '공창에 남긴 첫 인사가 둘 사이의 거리를 조금 좁힐 수 있습니다.',
    '공창에 남긴 첫 인사가 오늘의 분위기를 부드럽게 바꿀 수 있습니다.',
    '공창에 남긴 첫 인사가 예상 밖의 케미를 만들 수 있습니다.',
    '공창에 남긴 첫 인사가 다음 대화의 좋은 이유가 됩니다.',
    '공창에 남긴 첫 인사가 조용한 관심을 더 선명하게 만듭니다.',
    '공창에 남긴 첫 인사가 누군가의 시선을 잠깐 붙잡습니다.',
    '공창에 남긴 첫 인사가 편안한 호감으로 남을 수 있습니다.',
    '공창에 남긴 첫 인사가 오늘 밤의 작은 사건이 됩니다.',
    '무심코 던진 농담이 CODE 안에서 새로운 설렘으로 이어질 수 있습니다.',
    '무심코 던진 농담이 누군가에게 오래 기억될 수 있습니다.',
    '무심코 던진 농담이 둘 사이의 거리를 조금 좁힐 수 있습니다.',
    '무심코 던진 농담이 오늘의 분위기를 부드럽게 바꿀 수 있습니다.',
    '무심코 던진 농담이 예상 밖의 케미를 만들 수 있습니다.',
    '무심코 던진 농담이 다음 대화의 좋은 이유가 됩니다.',
    '무심코 던진 농담이 조용한 관심을 더 선명하게 만듭니다.',
    '무심코 던진 농담이 누군가의 시선을 잠깐 붙잡습니다.',
    '무심코 던진 농담이 편안한 호감으로 남을 수 있습니다.',
    '무심코 던진 농담이 오늘 밤의 작은 사건이 됩니다.',
    '살짝 늦은 답장이 CODE 안에서 새로운 설렘으로 이어질 수 있습니다.',
    '살짝 늦은 답장이 누군가에게 오래 기억될 수 있습니다.',
    '살짝 늦은 답장이 둘 사이의 거리를 조금 좁힐 수 있습니다.',
    '살짝 늦은 답장이 오늘의 분위기를 부드럽게 바꿀 수 있습니다.',
    '살짝 늦은 답장이 예상 밖의 케미를 만들 수 있습니다.',
    '살짝 늦은 답장이 다음 대화의 좋은 이유가 됩니다.',
    '살짝 늦은 답장이 조용한 관심을 더 선명하게 만듭니다.',
    '살짝 늦은 답장이 누군가의 시선을 잠깐 붙잡습니다.',
    '살짝 늦은 답장이 편안한 호감으로 남을 수 있습니다.',
    '살짝 늦은 답장이 오늘 밤의 작은 사건이 됩니다.',
    '닉네임을 부른 순간이 CODE 안에서 새로운 설렘으로 이어질 수 있습니다.',
    '닉네임을 부른 순간이 누군가에게 오래 기억될 수 있습니다.',
    '닉네임을 부른 순간이 둘 사이의 거리를 조금 좁힐 수 있습니다.',
    '닉네임을 부른 순간이 오늘의 분위기를 부드럽게 바꿀 수 있습니다.',
    '닉네임을 부른 순간이 예상 밖의 케미를 만들 수 있습니다.',
    '닉네임을 부른 순간이 다음 대화의 좋은 이유가 됩니다.',
    '닉네임을 부른 순간이 조용한 관심을 더 선명하게 만듭니다.',
    '닉네임을 부른 순간이 누군가의 시선을 잠깐 붙잡습니다.',
    '닉네임을 부른 순간이 편안한 호감으로 남을 수 있습니다.',
    '닉네임을 부른 순간이 오늘 밤의 작은 사건이 됩니다.',
    '가볍게 웃어준 리액션이 CODE 안에서 새로운 설렘으로 이어질 수 있습니다.',
    '가볍게 웃어준 리액션이 누군가에게 오래 기억될 수 있습니다.',
    '가볍게 웃어준 리액션이 둘 사이의 거리를 조금 좁힐 수 있습니다.',
    '가볍게 웃어준 리액션이 오늘의 분위기를 부드럽게 바꿀 수 있습니다.',
    '가볍게 웃어준 리액션이 예상 밖의 케미를 만들 수 있습니다.',
    '가볍게 웃어준 리액션이 다음 대화의 좋은 이유가 됩니다.',
    '가볍게 웃어준 리액션이 조용한 관심을 더 선명하게 만듭니다.',
    '가볍게 웃어준 리액션이 누군가의 시선을 잠깐 붙잡습니다.',
    '가볍게 웃어준 리액션이 편안한 호감으로 남을 수 있습니다.',
    '가볍게 웃어준 리액션이 오늘 밤의 작은 사건이 됩니다.',
    '밤에 이어진 짧은 대화가 CODE 안에서 새로운 설렘으로 이어질 수 있습니다.',
    '밤에 이어진 짧은 대화가 누군가에게 오래 기억될 수 있습니다.',
    '밤에 이어진 짧은 대화가 둘 사이의 거리를 조금 좁힐 수 있습니다.',
    '밤에 이어진 짧은 대화가 오늘의 분위기를 부드럽게 바꿀 수 있습니다.',
    '밤에 이어진 짧은 대화가 예상 밖의 케미를 만들 수 있습니다.',
    '밤에 이어진 짧은 대화가 다음 대화의 좋은 이유가 됩니다.',
    '밤에 이어진 짧은 대화가 조용한 관심을 더 선명하게 만듭니다.',
    '밤에 이어진 짧은 대화가 누군가의 시선을 잠깐 붙잡습니다.',
    '밤에 이어진 짧은 대화가 편안한 호감으로 남을 수 있습니다.',
    '밤에 이어진 짧은 대화가 오늘 밤의 작은 사건이 됩니다.',
    '처음 보는 사람과의 한마디가 CODE 안에서 새로운 설렘으로 이어질 수 있습니다.',
    '처음 보는 사람과의 한마디가 누군가에게 오래 기억될 수 있습니다.',
    '처음 보는 사람과의 한마디가 둘 사이의 거리를 조금 좁힐 수 있습니다.',
    '처음 보는 사람과의 한마디가 오늘의 분위기를 부드럽게 바꿀 수 있습니다.',
    '처음 보는 사람과의 한마디가 예상 밖의 케미를 만들 수 있습니다.',
    '처음 보는 사람과의 한마디가 다음 대화의 좋은 이유가 됩니다.',
    '처음 보는 사람과의 한마디가 조용한 관심을 더 선명하게 만듭니다.',
    '처음 보는 사람과의 한마디가 누군가의 시선을 잠깐 붙잡습니다.',
    '처음 보는 사람과의 한마디가 편안한 호감으로 남을 수 있습니다.',
    '처음 보는 사람과의 한마디가 오늘 밤의 작은 사건이 됩니다.',
    '익숙한 사람의 새로운 말투가 CODE 안에서 새로운 설렘으로 이어질 수 있습니다.',
    '익숙한 사람의 새로운 말투가 누군가에게 오래 기억될 수 있습니다.',
    '익숙한 사람의 새로운 말투가 둘 사이의 거리를 조금 좁힐 수 있습니다.',
    '익숙한 사람의 새로운 말투가 오늘의 분위기를 부드럽게 바꿀 수 있습니다.',
    '익숙한 사람의 새로운 말투가 예상 밖의 케미를 만들 수 있습니다.',
    '익숙한 사람의 새로운 말투가 다음 대화의 좋은 이유가 됩니다.',
    '익숙한 사람의 새로운 말투가 조용한 관심을 더 선명하게 만듭니다.',
    '익숙한 사람의 새로운 말투가 누군가의 시선을 잠깐 붙잡습니다.',
    '익숙한 사람의 새로운 말투가 편안한 호감으로 남을 수 있습니다.',
    '익숙한 사람의 새로운 말투가 오늘 밤의 작은 사건이 됩니다.',
    '조용히 맞은 타이밍이 CODE 안에서 새로운 설렘으로 이어질 수 있습니다.',
    '조용히 맞은 타이밍이 누군가에게 오래 기억될 수 있습니다.',
    '조용히 맞은 타이밍이 둘 사이의 거리를 조금 좁힐 수 있습니다.',
    '조용히 맞은 타이밍이 오늘의 분위기를 부드럽게 바꿀 수 있습니다.',
    '조용히 맞은 타이밍이 예상 밖의 케미를 만들 수 있습니다.',
    '조용히 맞은 타이밍이 다음 대화의 좋은 이유가 됩니다.',
    '조용히 맞은 타이밍이 조용한 관심을 더 선명하게 만듭니다.',
    '조용히 맞은 타이밍이 누군가의 시선을 잠깐 붙잡습니다.',
    '조용히 맞은 타이밍이 편안한 호감으로 남을 수 있습니다.',
    '조용히 맞은 타이밍이 오늘 밤의 작은 사건이 됩니다.',
    '장난스럽게 건넨 질문이 CODE 안에서 새로운 설렘으로 이어질 수 있습니다.',
    '장난스럽게 건넨 질문이 누군가에게 오래 기억될 수 있습니다.',
    '장난스럽게 건넨 질문이 둘 사이의 거리를 조금 좁힐 수 있습니다.',
    '장난스럽게 건넨 질문이 오늘의 분위기를 부드럽게 바꿀 수 있습니다.',
    '장난스럽게 건넨 질문이 예상 밖의 케미를 만들 수 있습니다.',
    '장난스럽게 건넨 질문이 다음 대화의 좋은 이유가 됩니다.',
    '장난스럽게 건넨 질문이 조용한 관심을 더 선명하게 만듭니다.',
    '장난스럽게 건넨 질문이 누군가의 시선을 잠깐 붙잡습니다.',
    '장난스럽게 건넨 질문이 편안한 호감으로 남을 수 있습니다.',
    '장난스럽게 건넨 질문이 오늘 밤의 작은 사건이 됩니다.',
]


def clean_cookie_message_pool(messages):
    cleaned = []
    seen = set()
    for message in messages:
        ...
        if "???" in text:
            continue
        last_word = text.rstrip().rstrip(".").split()[-1] if text.split() else ""
...
FORTUNE_COOKIE_MESSAGES = clean_cookie_message_pool(FORTUNE_COOKIE_MESSAGES)
CODE_COOKIE_MESSAGES = clean_cookie_message_pool(CODE_COOKIE_MESSAGES)
...
MENU_RECOMMENDATIONS = [
    "김치찌개", "된장찌개", "순두부찌개", "부대찌개", "청국장", "갈비탕", "설렁탕", "곰탕", "육개장", "감자탕",
    "뼈해장국", "순댓국", "콩나물국밥", "돼지국밥", "소고기국밥", "제육볶음", "오징어볶음", "낙지볶음", "불고기",
    "갈비찜", "찜닭", "닭볶음탕", "보쌈", "족발", "삼겹살", "목살구이", "돼지갈비", "닭갈비", "곱창전골", "막창구이",
    "비빔밥", "돌솥비빔밥", "김밥", "참치김밥", "충무김밥", "떡볶이", "라볶이", "순대", "튀김", "어묵탕",
    "칼국수", "수제비", "잔치국수", "비빔국수", "냉면", "막국수", "쫄면", "김치볶음밥", "새우볶음밥", "오므라이스",
    "짜장면", "짬뽕", "탕수육", "깐풍기", "마파두부덮밥", "고추잡채밥", "마라탕", "마라샹궈", "양꼬치", "꿔바로우",
    "초밥", "연어덮밥", "회덮밥", "돈카츠", "카츠동", "규동", "라멘", "우동", "소바", "오코노미야키",
    "파스타", "크림파스타", "토마토파스타", "알리오올리오", "리조또", "피자", "스테이크", "함박스테이크", "햄버거",
    "샌드위치", "샐러드", "포케", "타코", "부리또", "쌀국수", "팟타이", "분짜", "카레라이스", "인도커리",
    "치킨", "양념치킨", "간장치킨", "닭강정", "피쉬앤칩스", "핫도그", "토스트", "라면", "비빔면", "만두", "군만두",
]


def menu_recommendation_text(user_name):
    menu = random.choice(MENU_RECOMMENDATIONS)
    return (
        "🍽️ 메뉴추천\n\n"
        f"{display_nickname(user_name)}님 오늘은\n"
        f"{menu} 어때요?"
    )
...
def clean_hunmin_initials(value):
    return re.sub(r"\s+", "", str(value or "").strip())


def valid_hunmin_initials(value):
    initials = clean_hunmin_initials(value)
    if not (2 <= len(initials) <= 5):
        return False
    return all(ch in HUNMIN_CHOSEONG for ch in initials)


def normalize_hunmin_word(value):
    word = re.sub(r"\s+", "", str(value or "").strip())
    word = re.sub(r"[^가-힣]", "", word)
    return word


def hunmin_word_initials(word):
    initials = []
    for ch in normalize_hunmin_word(word):
        code = ord(ch)
        if 0xAC00 <= code <= 0xD7A3:
            initials.append(HUNMIN_CHOSEONG[(code - 0xAC00) // 588])
...
def active_hunmin_initial_rows():
    ...
    cur.execute("""
    SELECT initial
    FROM hunmin_initials
    WHERE is_active = 1
    ORDER BY initial ASC
    """)
    rows = cur.fetchall()
    conn.close()
    return [row["initial"] for row in rows]


def hunmin_initial_is_active(initials):
    conn = db()
    cur = conn.cursor()
    cur.execute("SELECT 1 FROM hunmin_initials WHERE initial = ? AND is_active = 1", (initials,))
    row = cur.fetchone()
    conn.close()
    return bool(row)


def hunmin_initials_text():
    rows = active_hunmin_initial_rows()
    if not rows:
        return "🔤 훈민정음 초성 목록\n\n등록된 초성이 없어요.\n운영방에서 /훈민초성추가 ㄱㅁ 으로 추가해 주세요."
    lines = ["🔤 훈민정음 초성 목록", ""]
    for idx, initial in enumerate(rows, 1):
        lines.append(f"{idx}. {initial}")
    return "\n".join(lines)


def add_hunmin_initial(initials, user_name):
    initials = clean_hunmin_initials(initials)
    if not valid_hunmin_initials(initials):
        return "초성은 ㄱ~ㅎ만 2~5글자로 입력해 주세요.\n예: /훈민초성추가 ㄱㅁ"
    conn = db()
    cur = conn.cursor()
    cur.execute("""
    INSERT INTO hunmin_initials (
        initial, is_active, created_by, created_at, updated_at
    ) VALUES (?, 1, ?, ?, ?)
    ON CONFLICT(initial)
    DO UPDATE SET
        is_active = 1,
        updated_at = excluded.updated_at
    """, (initials, user_name, now_str(), now_str()))
    conn.commit()
    conn.close()
    return f"✅ 훈민정음 초성 추가 완료\n\n{initials}"


def remove_hunmin_initial(initials):
    initials = clean_hunmin_initials(initials)
    if not valid_hunmin_initials(initials):
        return "삭제할 초성을 정확히 입력해 주세요.\n예: /훈민초성삭제 ㄱㅁ"
    conn = db()
    cur = conn.cursor()
    cur.execute("UPDATE hunmin_initials SET is_active = 0, updated_at = ? WHERE initial = ?", (now_str(), initials))
    changed = cur.rowcount
    conn.commit()
    conn.close()
    return f"✅ 훈민정음 초성 삭제 완료\n\n{initials}" if changed else "해당 초성을 찾지 못했어요."


def active_hunmin_game(source_id):
    conn = db()
    cur = conn.cursor()
    cur.execute("""
    SELECT *
...
def hunmin_game_result_text(game_id):
    conn = db()
    cur = conn.cursor()
    cur.execute("SELECT initials FROM hunmin_games WHERE id = ?", (game_id,))
    game = cur.fetchone()
    if not game:
        conn.close()
        return "🔤 훈민정음 종료\n\n게임 정보를 찾지 못했어요."
    ...
    if not rows:
        lines += ["정답자가 없었어요.", "다시 한 판 가보자."]
        return "\n".join(lines)

    top_score = int(rows[0]["score"] or 0)
    winners = [row for row in rows if int(row["score"] or 0) == top_score]
    if len(winners) == 1:
        winner = winners[0]
        lines += [
            f"우승: {display_nickname(winner['user_name'])}님 ({top_score}개)",
            "",
            "한 사람을 지목해 주세요.",
        ]
    else:
        names = ", ".join(f"{display_nickname(row['user_name'])}님" for row in winners)
        lines += [
            f"공동 1등: {names} ({top_score}개)",
            "",
            "공동 1등끼리 깔끔하게 가위바위보 가자.",
        ]
...
def expire_stale_hunmin_games(source_id):
    ...
    for row in rows:
        try:
            started_dt = datetime.strptime(row["started_at"], "%Y-%m-%d %H:%M:%S").replace(tzinfo=KST)
        except Exception:
            continue
        if datetime.now(KST) - started_dt >= timedelta(seconds=HUNMIN_GAME_SECONDS):
            expired_ids.append(int(row["id"]))
    ...
    conn.close()
    for game_id in expired_ids:
        notices.append(hunmin_game_result_text(game_id))
    return notices


def hunmin_game_start(source_id, user_id, user_name, initials=""):
    if source_id not in count_source_ids():
        return "훈민정음은 공창이나 운영방에서 사용할 수 있어요."

    initials = clean_hunmin_initials(initials)
    if not initials:
        pool = active_hunmin_initial_rows()
        if not pool:
            return "등록된 훈민정음 초성이 없어요.\n운영방에서 /훈민초성추가 ㄱㅁ 으로 추가해 주세요."
        initials = random.choice(pool)

    if not valid_hunmin_initials(initials):
        return "초성은 ㄱ~ㅎ만 2~5글자로 입력해 주세요.\n예: /훈민정음 ㄱㅁ"
    if not hunmin_initial_is_active(initials):
        return "등록되지 않은 초성이에요.\n운영방에서 /훈민초성목록 을 확인하거나 /훈민초성추가 로 추가해 주세요."
    ...
    """, (now_str(), source_id))
    cur.execute("""
    INSERT INTO hunmin_games (
        source_id, initials, status, started_by_user_id, started_by_user_name, started_at
    ) VALUES (?, ?, 'active', ?, ?, ?)
    """, (source_id, initials, user_id, user_name, now_str()))
    conn.commit()
    conn.close()
...
def process_hunmin_answer(source_id, user_id, user_name, text_value):
    if not user_id or source_id not in count_source_ids():
        return None
    if str(text_value or "").strip().startswith("/"):
        return None

    game = active_hunmin_game(source_id)
    if not game:
        return None

    try:
        started_dt = datetime.strptime(game["started_at"], "%Y-%m-%d %H:%M:%S").replace(tzinfo=KST)
    except Exception:
        return None
    if datetime.now(KST) - started_dt >= timedelta(seconds=HUNMIN_GAME_SECONDS):
        return None

    word = normalize_hunmin_word(text_value)
    ...
    """, (game["id"], user_id))
    score = int((cur.fetchone() or {"score": 0})["score"] or 0)
    conn.commit()
    conn.close()

    return f"✅ {word} 인정\n{display_nickname(user_name)}님 현재 {score}개"


def add_hunmin_blocked_word(arg, user_id, user_name):
    ...
    if len(word) < 2:
        return "막을 단어는 2글자 이상으로 입력해 주세요."
    conn = db()
    cur = conn.cursor()
    cur.execute("""
    ...
    return f"🚫 안되는 단어 등록 완료\n\n{word}{detail}"


def update_hunmin_blocked_word(arg, user_id, user_name):
    parts = str(arg or "").strip().split(maxsplit=1)
    if len(parts) < 2:
        return "사용법: /안되는단어수정 기존단어 새단어\n예: /안되는단어수정 김미트 미트"
    old_word = normalize_hunmin_word(parts[0])
    new_word = normalize_hunmin_word(parts[1])
    if len(old_word) < 2 or len(new_word) < 2:
        return "기존 단어와 새 단어를 2글자 이상으로 입력해 주세요."
    conn = db()
    cur = conn.cursor()
    cur.execute("SELECT reason FROM hunmin_blocked_words WHERE word = ?", (old_word,))
    old = cur.fetchone()
    reason = old["reason"] if old else ""
    cur.execute("DELETE FROM hunmin_blocked_words WHERE word = ?", (old_word,))
    cur.execute("""
    INSERT INTO hunmin_blocked_words (
        word, reason, created_by_user_id, created_by_user_name, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?)
    ...
    DO UPDATE SET
        reason = excluded.reason,
        updated_at = excluded.updated_at
    """, (new_word, reason, user_id, user_name, now_str(), now_str()))
    cur.execute("DELETE FROM hunmin_answers WHERE word IN (?, ?)", (old_word, new_word))
    conn.commit()
    conn.close()
    return f"✅ 안되는 단어 수정 완료\n\n{old_word} → {new_word}"


def delete_hunmin_blocked_word(arg):
    word = normalize_hunmin_word(arg)
    if len(word) < 2:
        return "사용법: /안되는단어삭제 단어"
    conn = db()
    cur = conn.cursor()
    ...
    conn.commit()
    conn.close()
    return f"✅ 안되는 단어 삭제 완료\n\n{word}" if changed else "해당 단어를 찾지 못했어요."


def hunmin_blocked_words_text():
    conn = db()
    cur = conn.cursor()
    cur.execute("""
    SELECT word, reason
    FROM hunmin_blocked_words
...
def inactive_user_match_rows(keyword, limit=5):
    query = normalize_mention_name(keyword)
    if len(query) < 2:
        return []
    ...
    cur = conn.cursor()
    cur.execute("""
    SELECT NULL AS deleted_id, user_name, 'inactive' AS status
    FROM users
...
def inactive_user_match_names(keyword, limit=5):
    return [row["name"] for row in inactive_user_match_rows(keyword, limit=limit)]
...
def dice_duel_target_not_found_text(keyword):
    ...
    inactive_rows = inactive_user_match_rows(keyword)
    if inactive_rows:
        lines = [
            "대결 상대를 찾지 못했습니다.",
            "",
        ...
        for row in inactive_rows:
            if row["status"] == "deleted" and row["deleted_id"]:
                lines.append(f"- #{row['deleted_id']} {row['name']} / 삭제유저")
                restore_hints.append(f"/삭제복구 #{row['deleted_id']}")
            else:
                lines.append(f"- {row['name']} / 비활성")
        if restore_hints:
            lines += ["", "복구 후 다시 듀얼을 걸어주세요.", restore_hints[0]]
        return (
            "\n".join(lines)
        )
...
def handle(event):
    ...
    if text == "/훈민정음" or text.startswith("/훈민정음 "):
        initials = text.replace("/훈민정음", "", 1).strip()
        msg = hunmin_game_start(source_id, user_id, user_name, initials)
    ...
    if text == "/안되는단어수정" or text.startswith("/안되는단어수정 "):
        msg = update_hunmin_blocked_word(text.replace("/안되는단어수정", "", 1).strip(), user_id, user_name)
        reply_many(event.reply_token, split_text_messages("\n\n".join(dict.fromkeys(public_notices + [msg]))))
        return

    if text == "/안되는단어삭제" or text.startswith("/안되는단어삭제 "):
        msg = delete_hunmin_blocked_word(text.replace("/안되는단어삭제", "", 1).strip())
        reply_many(event.reply_token, split_text_messages("\n\n".join(dict.fromkeys(public_notices + [msg]))))
        return

    if text == "/안되는단어목록":
        reply_many(event.reply_token, split_text_messages("\n\n".join(dict.fromkeys(public_notices + [hunmin_blocked_words_text()]))))
        return
    ...
    if text == "/훈민초성목록":
        if not is_operator_room(source_id):
            reply(event.reply_token, operator_only_warning())
            return
        reply_many(event.reply_token, split_text_messages(hunmin_initials_text()))
        return

    if text == "/훈민초성추가" or text.startswith("/훈민초성추가 "):
        if not is_operator_room(source_id):
            reply(event.reply_token, operator_only_warning())
            return
        msg = add_hunmin_initial(text.replace("/훈민초성추가", "", 1).strip(), user_name)
        reply(event.reply_token, msg)
        return

    if text == "/훈민초성삭제" or text.startswith("/훈민초성삭제 "):
        if not is_operator_room(source_id):
            reply(event.reply_token, operator_only_warning())
            return
        msg = remove_hunmin_initial(text.replace("/훈민초성삭제", "", 1).strip())
        reply(event.reply_token, msg)
        return
    ...
    if text == "/메뉴추천":
        reply(event.reply_token, menu_recommendation_text(user_name))
        return
